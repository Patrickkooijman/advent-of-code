...
export class Day2 extends Day {
  ...
  challengeOneHandler = (input: string): number => {
    return input
      ...
      .map(
        ([a, _, b]: string[]): Match => [
          OpponentShape[a as keyof typeof OpponentShape],
          PlayerShape[b as keyof typeof PlayerShape],
        ]
      )
      .reduce(
        (acc: number, [opponent, player]: Match): number =>
          acc + GameScore[this.toGameResult(opponent, player)] + ShapeScore[player],
        0
      );
  };
  ...
  challengeTwoHandler = (input: string): number => {
    return input
      ...
      .filter(([a, b]) => !!a && !!b)
      .map(
        ([a, _, b]: string[]): MatchResult => [
          OpponentShape[a as keyof typeof OpponentShape],
          ExpectedResult[b as keyof typeof ExpectedResult],
        ]
      )
      .reduce(
        (acc: number, [opponent, result]: MatchResult): number =>
          acc + GameScore[result] + ShapeScore[this.toExpectedShape(opponent, result)],
        0
      );
  };
