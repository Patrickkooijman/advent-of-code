...
export default class Day13 extends Day {
    ...
    challengeOneHandler = (input: string): number => {
        ...
        return results.reduce((acc: number, curr: boolean | undefined, index: number) => acc + (curr ? index + 1 : 0) ,0)
    }
    challengeTwoHandler = (input: string): number => {
        ...
        const pairs: Array<Pair> = this.parseInput(input);
        const packets: number[][][] = [ firstAdditionalPackage, secondAdditionalPackage ].concat(pairs.flatMap(({ left, right}) => [left, right]));

        this.sortPackets(packets);

        const firstAdditionalPackageIndex: number = packets.findIndex(packet => packet === firstAdditionalPackage) + 1;
        const secondAdditionalPackageIndex: number = packets.findIndex(packet => packet === secondAdditionalPackage) + 1;
    ...
    private isInCorrectOrder(left: number | number[] | number[][], right: number | number[] | number[][]): boolean | undefined {
    ...
    private parseInput = (input: string): Array<Pair> => input.split(/\n\s*\n/).map((line: string): Pair => {
