...
export default class Day9 extends Day {
    ...
    private calculateTouchedFields = (input: string, amountOfKnots: number) => {
        ...
        let knots: Coordinate[] = Array(amountOfKnots + 1).fill(null).map(() => ({ x: 0, y: 0, }));
    ...
    private isTailWithinRange = (head: Coordinate, tail: Coordinate): boolean => Math.abs(head.x - tail.x) <= 1 && Math.abs(head.y - tail.y) <= 1;
    private isNextTo = (head: Coordinate, tail: Coordinate): boolean => (head.x === tail.x &&  Math.abs(head.y - tail.y) <= 1 )  || (head.y === tail.y && Math.abs(head.x - tail.x) <= 1 ) ;
    private moveTailTo = (head: Coordinate, tail: Coordinate): Coordinate => {
        ...
        return options.find((option: Coordinate): boolean => !!option && this.isNextTo(head, option)) ||  options.find((option: Coordinate): boolean => !!option && this.isTailWithinRange(head, option)) ||  { x: 0, y: 0};
    }

    private calculateResult = (tails: Array<Coordinate>): number =>
        tails
            .filter((tail: Coordinate, index:number, self: Coordinate[]): boolean =>
                self.findIndex((coor: Coordinate): boolean => coor.x === tail.x && coor.y === tail.y) === index
            ).length;
}
