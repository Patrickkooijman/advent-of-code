...
export default class Day14 extends Day {
    ...
    challengeTwoHandler = (input: string): number => this.countDroppedSandsPartTwo(this.parseInput(input));
    ...
    private parseInput = (input: string): Data => {
        ...
        lines.forEach((line: string) => line.split('->')
            .forEach((from: string, index:number, self: string[]): void => {
