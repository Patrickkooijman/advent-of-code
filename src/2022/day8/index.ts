...
export default class Day8 extends Day {
  ...
  challengeOneHandler = (input: string): number => {
    ...
    return trees
      .flatMap(t => t)
      .map(tree => this.treeIsVisible(tree, trees))
  ...
  private treeIsVisible = (tree: Tree, trees: Tree[][]): boolean => {
    ...
    const above: number[] = trees
      .slice(0, yPos)
      .map(t => t[xPos])
      .map(t => t!.height);
    const below: number[] = trees
      .slice(yPos + 1)
      .map(t => t[xPos])
      .filter(Boolean)
      .map(t => t!.height);
  ...
  private scenicScore = (tree: Tree, trees: Tree[][]): number => {
    ...
    const above: Tree[] = trees
      .slice(0, yPos)
      .map(t => t[xPos]!)
      .reverse();
    const below: Tree[] = trees
      .slice(yPos + 1)
      .map(t => t[xPos]!)
      .filter(Boolean);
    ...
    return [left, right, above, below]
      .map(view =>
        Math.max(...view.map(t => t.height)) < height
          ? view.length
