...
export default class Day11 extends Day {
    ...
    handle = (input: string, amountOfRounds: number, challenge: number): number => {
        ...
        const divisionMethod: WorryDivision = challenge === 1 ? this.challengeOneWorryLevelMethod : this.challengeTwoWorryLevelMethod(monkeys);
        ...
        const [a = 0, b = 0] = Array.from(monkeys).map(([_, v]) => v.inspections).sort((a, b) => b - a);

        return  a * b;
    }

    private challengeOneWorryLevelMethod: WorryDivision = (worryLevel: number): number => Math.floor(worryLevel / 3);
    private challengeTwoWorryLevelMethod = (monkeys: Map<string, Monkey>): WorryDivision => {
        const productOfDivisions: number = Array.from(monkeys).map(([_, monkey]) => monkey.test).reduce((a, c) => a * c, 1);
    ...
    parseInput = (input: string): Map<string, Monkey> => {
        ...
        input
            ...
            .forEach((monkey: string[]): void => {
                ...
                const whenTrueMonkey: Monkey = monkeys.has(whenTrueMonkeyName) ? monkeys.get(whenTrueMonkeyName) : monkeys.set(whenTrueMonkeyName, new Monkey(whenTrueMonkeyName)).get(whenTrueMonkeyName);

                const whenFalseMonkeyNumber = wf!.match(/(\d+)/g)!.map(Number).at(0);
                const whenFalseMonkeyName = `Monkey ${whenFalseMonkeyNumber}`;
                const whenFalseMonkey: Monkey = monkeys.has(whenFalseMonkeyName) ? monkeys.get(whenFalseMonkeyName) : monkeys.set(whenFalseMonkeyName, new Monkey(whenFalseMonkeyName)).get(whenFalseMonkeyName);

                const test = t!.match(/(\d+)/g)!.map(Number).at(0) || 1;

                if (!monkeys.has(name)) {
                    monkeys.set(name, new Monkey(name, items, operation, test, whenTrueMonkey, whenFalseMonkey));
                } else {
...
class Monkey {
    ...
    constructor(name: string, items?: Array<number>, operation?: Operation, test?: number, whenTrue?: Monkey, whenFalse?: Monkey) {
        this.name = name;
