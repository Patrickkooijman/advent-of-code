...
export const isNiceString = (value: string): boolean => createPipe(
    hasThreeVowels,
...
export const challengeTwoHandler = (input: string): number => abstractHandler(input, isNiceAdvancedString);
export const isNiceAdvancedString = (value: string): boolean => createPipe(
    doesRepeat,
    isSpaced
)(value);
...
const abstractHandler = (input: string, withMethod: Predicate): number =>
    input
        ...
        .filter(withMethod)
        .length;
const createPipe = (...fns: Function[]): Function => (input: string): boolean =>
    fns.reduce((a: boolean, f: Function) => a && f(input), true);

const hasThreeVowels = (input: string): boolean => (input.match(/[aeiou]/gi) || []).length >= 3;
const hasTwoConsecutiveLetters = (input: string): boolean => !!(input.match(/([a-z])\1/) || []).length;
const doesNotContainBlacklisted = (input: string): boolean => !(input.match(/ab|cd|pq|xy/) || []).length;
