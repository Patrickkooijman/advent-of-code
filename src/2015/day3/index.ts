...
export const challengeOne = (): Promise<string> =>
  fetch('3')
    ...
    .then(res => `Day 3, challenge 1: ${res}`);
...
export const challengeOneHandler = (input: string): number => {
  ...
  return [startLocation]
    .concat(
      input.split('').map((location: string): Location => {
        switch (location) {
          case '<':
...
export const challengeTwo = (): Promise<string> =>
  fetch('3')
    ...
    .then(res => `Day 3, challenge 2: ${res}`);
      ...
      input.split('').map((location: string): Location => {
        switch (true) {
          case location === '<' && isSanta:
            santaLastLocation = {
              x: santaLastLocation.x - 1,
              y: santaLastLocation.y,
            };
            isSanta = !isSanta;
            return santaLastLocation;
          case location === '<' && !isSanta:
            roboSantaLastLocation = {
              x: roboSantaLastLocation.x - 1,
              y: roboSantaLastLocation.y,
            };
            isSanta = !isSanta;
          ...
          case location === '^' && isSanta:
            santaLastLocation = {
              x: santaLastLocation.x,
              y: santaLastLocation.y + 1,
            };
            isSanta = !isSanta;
            return santaLastLocation;
          case location === '^' && !isSanta:
            roboSantaLastLocation = {
              x: roboSantaLastLocation.x,
              y: roboSantaLastLocation.y + 1,
            };
            isSanta = !isSanta;
            return roboSantaLastLocation;
